"""
KiriKiri Processor
Handles translation for KiriKiri games (.ks scripts and .xp3 archives)
"""

import os
import re
import struct
import zlib
from typing import Dict, List, Tuple, Any
import tempfile
import shutil


class KiriKiriProcessor:
    """Processor for KiriKiri engine games"""
    
    def __init__(self):
        self.supported_extensions = ['.ks', '.xp3', '.tjs']
        
        # KiriKiri script patterns
        self.text_patterns = [
            # Message text in brackets
            r'\[([^\]]*[ひらがなカタカナ漢字々〆ヵヶ一-龯][^\]]*)\]',
            
            # Message text in quotes
            r'"([^"]*[ひらがなカタカナ漢字々〆ヵヶ一-龯][^"]*)"',
            r"'([^']*[ひらがなカタカナ漢字々〆ヵヶ一-龯][^']*)'",
            
            # Character name
            r'@name\s*=\s*"([^"]*)"',
            r'@name\s*=\s*\'([^\']*)\'',
            
            # Message commands
            r'@mes\s*=\s*"([^"]*)"',
            r'@mes\s*=\s*\'([^\']*)\'',
            
            # Choice text
            r'@choice\s*=\s*"([^"]*)"',
            r'@choice\s*=\s*\'([^\']*)\'',
            
            # Text variables
            r'\$([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*"([^"]*[ひらがなカタカナ漢字々〆ヵヶ一-龯][^"]*)"',
            
            # Ruby text (furigana)
            r'@ruby\s*text\s*=\s*"([^"]*)"',
            
            # Graph text
            r'@graph\s*storage\s*=\s*"([^"]*)".*?@text\s*=\s*"([^"]*)"',
            
            # Simple Japanese text lines
            r'^([ひらがなカタカナ漢字々〆ヵヶ一-龯][^\n\r]*)',
        ]
        
        # XP3 archive magic
        self.xp3_magic = b'XP3\r\n \n\x1a\x8b\x67\x01'
    
    def detect_kirikiri_project(self, directory: str) -> bool:
        """Detect if directory contains a KiriKiri project"""
        
        # Check for KiriKiri indicators
        kirikiri_indicators = [
            'data.xp3',
            'patch.xp3',
            'voice.xp3',
            'bgm.xp3',
            'scenario.xp3',
            'startup.tjs',
            'Config.tjs',
            'system.ks'
        ]
        
        for indicator in kirikiri_indicators:
            if os.path.exists(os.path.join(directory, indicator)):
                return True
        
        # Check for .ks files
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.lower().endswith(('.ks', '.xp3', '.tjs')):
                    return True
        
        return False
    
    def find_kirikiri_text_files(self, directory: str) -> List[str]:
        """Find all KiriKiri text files"""
        
        text_files = []
        
        # Search for script files
        for root, dirs, files in os.walk(directory):
            for file in files:
                file_path = os.path.join(root, file)
                
                if file.lower().endswith('.ks'):
                    if self.might_contain_japanese(file_path):
                        text_files.append(file_path)
                
                elif file.lower().endswith('.xp3'):
                    text_files.append(file_path)
                
                elif file.lower().endswith('.tjs'):
                    if self.might_contain_japanese(file_path):
                        text_files.append(file_path)
        
        return text_files
    
    def might_contain_japanese(self, file_path: str) -> bool:
        """Check if file might contain Japanese text"""
        try:
            # Try UTF-8 first
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read(1024)
        except Exception:
            try:
                # Try Shift-JIS
                with open(file_path, 'r', encoding='shift_jis', errors='ignore') as f:
                    content = f.read(1024)
            except Exception:
                return False
        
        japanese_pattern = r'[ひらがなカタカナ漢字々〆ヵヶ一-龯]'
        return bool(re.search(japanese_pattern, content))
    
    def extract_translatable_text(self, file_path: str) -> List[Tuple[str, str, str]]:
        """Extract translatable text from KiriKiri files"""
        
        if file_path.lower().endswith('.xp3'):
            return self._extract_from_xp3_archive(file_path)
        else:
            return self._extract_from_script_file(file_path)
    
    def _extract_from_script_file(self, file_path: str) -> List[Tuple[str, str, str]]:
        """Extract text from .ks or .tjs files"""
        
        texts = []
        
        try:
            # Try different encodings
            content = None
            for encoding in ['utf-8', 'shift_jis', 'cp932']:
                try:
                    with open(file_path, 'r', encoding=encoding) as f:
                        content = f.read()
                    break
                except UnicodeDecodeError:
                    continue
            
            if not content:
                return texts
            
            lines = content.split('\n')
            
            for line_num, line in enumerate(lines, 1):
                line = line.strip()
                if not line or line.startswith(';'):  # Skip comments
                    continue
                
                # Try each pattern
                for pattern in self.text_patterns:
                    matches = re.findall(pattern, line, re.MULTILINE)
                    
                    if isinstance(matches[0] if matches else None, tuple):
                        # Handle patterns that return tuples
                        for match_tuple in matches:
                            for match in match_tuple:
                                if self._is_japanese_text(match):
                                    key = f"line_{line_num}_{len(texts)}"
                                    texts.append((key, match, file_path))
                    else:
                        # Handle patterns that return strings
                        for match in matches:
                            if self._is_japanese_text(match):
                                key = f"line_{line_num}_{len(texts)}"
                                texts.append((key, match, file_path))
        
        except Exception as e:
            print(f"Error extracting from KiriKiri file {file_path}: {e}")
        
        return texts
    
    def _extract_from_xp3_archive(self, file_path: str) -> List[Tuple[str, str, str]]:
        """Extract text from .xp3 archive files"""
        
        texts = []
        temp_dir = None
        
        try:
            # Extract xp3 archive
            temp_dir = tempfile.mkdtemp(prefix="xp3_extract_")
            
            if self._extract_xp3_archive(file_path, temp_dir):
                # Process extracted files
                for root, dirs, files in os.walk(temp_dir):
                    for file in files:
                        if file.lower().endswith(('.ks', '.tjs')):
                            extracted_file = os.path.join(root, file)
                            file_texts = self._extract_from_script_file(extracted_file)
                            texts.extend(file_texts)
            
        except Exception as e:
            print(f"Error extracting XP3 archive {file_path}: {e}")
        
        finally:
            # Cleanup
            if temp_dir and os.path.exists(temp_dir):
                shutil.rmtree(temp_dir, ignore_errors=True)
        
        return texts
    
    def _extract_xp3_archive(self, xp3_file: str, output_dir: str) -> bool:
        """Extract XP3 archive file"""
        
        try:
            with open(xp3_file, 'rb') as f:
                # Check magic number
                magic = f.read(11)
                if magic != self.xp3_magic:
                    return False
                
                # Skip to index offset
                f.seek(-8, 2)  # Go to end of file - 8 bytes
                index_offset = struct.unpack('<Q', f.read(8))[0]
                
                # Read index
                f.seek(index_offset)
                
                # This is a simplified XP3 extraction
                # Real XP3 files have complex compression and encryption
                # For now, we'll implement basic extraction
                
                while True:
                    try:
                        # Read entry header
                        entry_magic = f.read(4)
                        if entry_magic != b'File':
                            break
                        
                        entry_size = struct.unpack('<I', f.read(4))[0]
                        entry_data = f.read(entry_size)
                        
                        # Parse entry (simplified)
                        if len(entry_data) > 16:
                            # Extract filename and data offset
                            # This is a very basic implementation
                            pass
                        
                    except struct.error:
                        break
                
                return True
                
        except Exception as e:
            print(f"Error extracting XP3 archive: {e}")
            return False
    
    def _is_japanese_text(self, text: str) -> bool:
        """Check if text contains Japanese characters"""
        if not text or len(text.strip()) < 2:
            return False
        
        japanese_pattern = r'[ひらがなカタカナ漢字々〆ヵヶ一-龯]'
        return bool(re.search(japanese_pattern, text))
    
    def create_translation_file(self, original_file: str, translations: Dict[str, str], 
                              output_dir: str, target_language: str):
        """Create translated file for KiriKiri"""
        
        # Determine output path
        rel_path = os.path.basename(original_file)
        lang_code = self._get_language_code(target_language)
        base_name = os.path.splitext(rel_path)[0]
        ext = os.path.splitext(rel_path)[1]
        
        output_file = os.path.join(output_dir, f"{base_name}_{lang_code}{ext}")
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        
        try:
            # Read original file
            content = None
            encoding_used = 'utf-8'
            
            for encoding in ['utf-8', 'shift_jis', 'cp932']:
                try:
                    with open(original_file, 'r', encoding=encoding) as f:
                        content = f.read()
                    encoding_used = encoding
                    break
                except UnicodeDecodeError:
                    continue
            
            if not content:
                return
            
            # Apply translations
            translated_content = content
            for key, translation in translations.items():
                # This is a simplified replacement
                # In practice, you'd want more sophisticated pattern matching
                original_texts = self.extract_translatable_text(original_file)
                for orig_key, orig_text, _ in original_texts:
                    if orig_key == key:
                        translated_content = translated_content.replace(orig_text, translation)
                        break
            
            # Save translated file
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(translated_content)
                
        except Exception as e:
            print(f"Error creating KiriKiri translation file: {e}")
    
    def _get_language_code(self, language: str) -> str:
        """Get language code for file naming"""
        language_codes = {
            'English': 'en',
            'Russian': 'ru',
            'Spanish': 'es',
            'French': 'fr',
            'German': 'de',
            'Chinese': 'zh',
            'Korean': 'ko'
        }
        return language_codes.get(language, 'en')
    
    def get_file_stats(self, file_path: str) -> Dict[str, Any]:
        """Get statistics for a KiriKiri file"""
        
        try:
            file_size = os.path.getsize(file_path)
            texts = self.extract_translatable_text(file_path)
            
            japanese_texts = [text for key, text, path in texts if self._is_japanese_text(text)]
            
            # Estimate tokens
            total_chars = sum(len(text) for text in japanese_texts)
            estimated_tokens = total_chars // 3
            
            needs_translation = len(japanese_texts) > 0
            
            return {
                'file_size': file_size,
                'total_text_entries': len(texts),
                'japanese_text_entries': len(japanese_texts),
                'estimated_tokens': estimated_tokens,
                'needs_translation': needs_translation,
                'file_type': 'KiriKiri'
            }
            
        except Exception as e:
            return {
                'file_size': 0,
                'total_text_entries': 0,
                'japanese_text_entries': 0,
                'estimated_tokens': 0,
                'needs_translation': False,
                'error': str(e)
            }
