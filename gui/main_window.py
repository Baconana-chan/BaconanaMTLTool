...
class SegmentRetranscriptionThread(QThread):
    ...
    def _generate_preview(self):
        ...
        preview = f"=== SEGMENT RETRANSCRIPTION PREVIEW ===\n"
        preview += f"Time Range: {self.start_time:.2f}s → {self.end_time:.2f}s\n"
        preview += f"Duration: {self.end_time - self.start_time:.2f}s\n"
        preview += f"Segments Found: {len(self.new_transcription.segments)}\n\n"
        ...
        for i, segment in enumerate(self.new_transcription.segments, 1):
            ...
            start_formatted = f"{segment.start:.1f}s"
            end_formatted = f"{segment.end:.1f}s"
            duration = segment.end - segment.start
            
            preview += f"{i:2d}. [{start_formatted} → {end_formatted}] ({duration:.1f}s)\n"
...
class MainWindow(QMainWindow):
    ...
    def format_transcription_preview(self, result):
        """Format transcription result for preview with numbering and timestamps"""
        if not result or not hasattr(result, 'segments') or not result.segments:
            return result.text if result else "No transcription available."
        
        formatted_lines = []
        formatted_lines.append("=== TRANSCRIPTION PREVIEW ===\n")
        
        for i, segment in enumerate(result.segments, 1):
            # Format timestamps
            start_time = self.format_time_for_subtitles(segment.start)
            end_time = self.format_time_for_subtitles(segment.end)
            
            # Add segment with number, timestamps, and text
            formatted_lines.append(f"{i:3d}. [{start_time} → {end_time}]")
        ...
        formatted_lines.append(f"Total segments: {len(result.segments)}")
        formatted_lines.append(f"Total duration: {self.format_time_for_subtitles(total_duration)}")
        formatted_lines.append("=" * 40)
        
        return "\n".join(formatted_lines)
    
    def format_time_for_subtitles(self, seconds):
        """Format time in MM:SS or HH:MM:SS format for subtitles"""
        if seconds is None:
            seconds = 0
    ...
    def format_raw_transcription(self, result):
        """Format transcription for raw text file with detailed information"""
        if not result:
            return "No transcription available."
            
        lines = []
        lines.append("# RAW TRANSCRIPTION OUTPUT")
        lines.append("# Generated by BaconanaMTL Tool")
        lines.append(f"# Total segments: {len(result.segments) if hasattr(result, 'segments') and result.segments else 0}")
        ...
        # If we have segments, add detailed breakdown
        if hasattr(result, 'segments') and result.segments:
            lines.append("## SEGMENTED TRANSCRIPTION:")
            lines.append("")
            ...
            for i, segment in enumerate(result.segments, 1):
                ...
                lines.append(f"[{i:03d}] {start_time} → {end_time} (Duration: {duration:.1f}s)")
...
• Chinese/Japanese models work best for Asian language translation
    ...
    def on_transcription_complete(self, result, provider):
        ...
        try:
            ...
            # Display preview with formatting
            formatted_preview = self.format_transcription_preview(result)
            self.transcription_preview.setText(formatted_preview)
            ...
            if self.save_raw_check.isChecked():
                ...
                with open(raw_path, 'w', encoding='utf-8') as f:
                    formatted_raw = self.format_raw_transcription(result)
